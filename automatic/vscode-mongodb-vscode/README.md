...
| `mdb.defaultSortOrder` | The default sort order when browsing documents in a collection. | `default` |
| `mdb.documentViewAndEditFormat` | The format in which documents and playground results are shown. | `shell` |
| `mdb.useWebViewDataBrowser` | Use a webview for viewing documents in a collection and find or aggregation playground results. When false, a VS Code editor is used. | `true` |
