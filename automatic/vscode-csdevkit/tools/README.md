...
Use the `Collect C# Dev Kit Logs` command from the Command Palette to gather diagnostic data whenever you need to investigate an issue or file a bug report. The command offers two modes:

* **Collect existing logs** — packages the current log files for C# Dev Kit, the .NET Install Tool, C#, and Razor into a ZIP archive along with a settings snapshot, environment information, and a README that explains what was collected.
* **Record a new trace** — runs a live `dotnet-trace` session against active C# Dev Kit processes. Reproduce the issue, click **Stop Recording** in the status bar, and the `.nettrace` files are saved to a folder of your choice.
