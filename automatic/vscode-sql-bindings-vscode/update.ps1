...
function global:au_GetLatest {
    ...
    $body = @{
        filters = @(
            @{
                criteria   = @(
                    @{ filterType = 7; value = "ms-mssql.sql-bindings-vscode" }
                )
                pageNumber = 1
                pageSize   = 1
            }
    ...
    $vsixUrl = "https://marketplace.visualstudio.com/_apis/public/gallery/publishers/ms-mssql/vsextensions/sql-bindings-vscode/$version/vspackage"
...
function global:au_BeforeUpdate {
    ...
    $vsixPath = Join-Path $toolsDir "ms-mssql.sql-bindings-vscode-$($Latest.Version).vsix"
...
function global:au_SearchReplace {
    @{
        "tools\chocolateyInstall.ps1" = @{
            "(?i)(ms-mssql\.sql-bindings-vscode-)[\d\.]+(\.vsix)" = "`${1}$($Latest.Version)`${2}"
        }
    }
}
