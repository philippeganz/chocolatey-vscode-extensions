...
function global:au_GetLatest {
    ...
    $vsixUrl = "https://marketplace.visualstudio.com/_apis/public/gallery/publishers/vscjava/vsextensions/vscode-java-test/$version/vspackage"
...
function global:au_BeforeUpdate {
    ...
    $vsixPath = Join-Path $toolsDir "vscjava.vscode-java-test-$($Latest.Version).vsix"
...
function global:au_SearchReplace {
    @{
        "tools\chocolateyInstall.ps1" = @{
            "(?i)(vscjava\.vscode-java-test-)[\d\.]+(\.vsix)" = "`${1}$($Latest.Version)`${2}"
        }
    }
}
