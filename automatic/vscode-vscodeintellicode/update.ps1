...
function global:au_GetLatest {
    ...
    $body = @{
        filters = @(
            @{
                criteria   = @(
                    @{ filterType = 7; value = "VisualStudioExptTeam.vscodeintellicode" }
                )
                pageNumber = 1
                pageSize   = 1
            }
    ...
    $vsixUrl = "https://marketplace.visualstudio.com/_apis/public/gallery/publishers/VisualStudioExptTeam/vsextensions/vscodeintellicode/$version/vspackage"
...
function global:au_BeforeUpdate {
    ...
    $vsixPath = Join-Path $toolsDir "VisualStudioExptTeam.vscodeintellicode-$($Latest.Version).vsix"
...
function global:au_SearchReplace {
    @{
        "tools\chocolateyInstall.ps1" = @{
            "(?i)(VisualStudioExptTeam\.vscodeintellicode-)[\d\.]+(\.vsix)" = "`${1}$($Latest.Version)`${2}"
        }
    }
}
