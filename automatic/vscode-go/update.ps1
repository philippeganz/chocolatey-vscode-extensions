...
function global:au_GetLatest {
    ...
    $vsixUrl = "https://marketplace.visualstudio.com/_apis/public/gallery/publishers/golang/vsextensions/go/$version/vspackage"
...
function global:au_BeforeUpdate {
    ...
    $vsixPath = Join-Path $toolsDir "golang.go-$($Latest.Version).vsix"
...
function global:au_SearchReplace {
    @{
        "tools\chocolateyInstall.ps1" = @{
            "(?i)(golang\.go-)[\d\.]+(\.vsix)" = "`${1}$($Latest.Version)`${2}"
        }
    }
}
