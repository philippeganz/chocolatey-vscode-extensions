...
function global:au_GetLatest {
    ...
    $body = @{
        filters = @(
            @{
                criteria   = @(
                    @{ filterType = 7; value = "ms-vscode.cpptools-extension-pack" }
                )
                pageNumber = 1
                pageSize   = 1
            }
    ...
    $vsixUrl = "https://marketplace.visualstudio.com/_apis/public/gallery/publishers/ms-vscode/vsextensions/cpptools-extension-pack/$version/vspackage"
...
function global:au_BeforeUpdate {
    ...
    $vsixPath = Join-Path $toolsDir "ms-vscode.cpptools-extension-pack-$($Latest.Version).vsix"
...
function global:au_SearchReplace {
    @{
        "tools\chocolateyInstall.ps1" = @{
            "(?i)(ms-vscode\.cpptools-extension-pack-)[\d\.]+(\.vsix)" = "`${1}$($Latest.Version)`${2}"
        }
    }
}
