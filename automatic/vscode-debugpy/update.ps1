...
function global:au_GetLatest {
    ...
    $body = @{
        filters = @(
            @{
                criteria   = @(
                    @{ filterType = 7; value = "ms-python.debugpy" }
                )
                pageNumber = 1
                pageSize   = 1
            }
    ...
    $vsixUrl = "https://marketplace.visualstudio.com/_apis/public/gallery/publishers/ms-python/vsextensions/debugpy/$version/vspackage"
...
function global:au_BeforeUpdate {
    ...
    $vsixPath = Join-Path $toolsDir "ms-python.debugpy-$($Latest.Version).vsix"
...
function global:au_SearchReplace {
    @{
        "tools\chocolateyInstall.ps1" = @{
            "(?i)(ms-python\.debugpy-)[\d\.]+(\.vsix)" = "`${1}$($Latest.Version)`${2}"
        }
    }
}
