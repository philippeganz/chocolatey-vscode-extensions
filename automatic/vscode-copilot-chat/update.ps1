...
function global:au_GetLatest {
    ...
    $body = @{
        filters = @(
            @{
                criteria   = @(
                    @{ filterType = 7; value = "GitHub.copilot-chat" }
                )
                pageNumber = 1
                pageSize   = 1
            }
    ...
    $vsixUrl = "https://marketplace.visualstudio.com/_apis/public/gallery/publishers/GitHub/vsextensions/copilot-chat/$version/vspackage"
...
function global:au_BeforeUpdate {
    ...
    $vsixPath = Join-Path $toolsDir "GitHub.copilot-chat-$($Latest.Version).vsix"
...
function global:au_SearchReplace {
    @{
        "tools\chocolateyInstall.ps1" = @{
            "(?i)(GitHub\.copilot-chat-)[\d\.]+(\.vsix)" = "`${1}$($Latest.Version)`${2}"
        }
    }
}
